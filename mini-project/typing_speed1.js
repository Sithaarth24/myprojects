...
inp_text.addEventListener('input',(event) =>{
    ...
    if(a[a.length - 1] == ' ')
    {
        ...
        const quote_word = quote_spans[i].innerText
        console.log(quote_word.slice(0,quote_word.length-1) + b[b.length-2])
        if(quote_word.slice(0,quote_word.length-1)==b[b.length-2])
        {
            c++;
